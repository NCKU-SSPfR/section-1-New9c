...
def _parse_map(map_string, map_size, reversal_nodes=[]):
    ...
    grid = [bin(ord(c))[2:].zfill(8) for c in filtered_chars]
    
    arr = []
    for line in grid:
        first_half = int(line[:4], 2)
        second_half = int(line[4:], 2)
        arr.extend([first_half % 2, second_half % 2])
    
    while len(arr) < width * height:
        arr.append(0)
    
    arr = arr[:width * height]
    
    parsed_map = np.array(arr).reshape((height, width))
    
    for x, y in reversal_nodes:
        if 0 <= x < height and 0 <= y < width:
            parsed_map[y, x] = 1 - parsed_map[y, x]
